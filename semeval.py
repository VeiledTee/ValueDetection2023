...
arguments = pd.read_csv("SemEval/data/arguments.tsv", sep='\t')
labels = pd.read_csv("SemEval/data/labels-level2.tsv", sep='\t')
...
bookkeeping = dict()
for i in range(len(thought_strings)):
    if f"{thought_strings[i].split('_')[0]}_token" not in bookkeeping.keys():
        bookkeeping[f"{thought_strings[i]}_token"] = 0
        bookkeeping[f"{thought_strings[i]}_cls"] = 0
    token_cosine = []
    cls_cosine = []
    flag = False
    if thought_strings[i].split("_")[0] in list(center_of_venn_diagram["Word 1"]):
        flag = True
    for j in range(len(thought_strings)):
        if i == j or thought_strings[i].split("_")[1] == thought_strings[j].split("_")[1]:  # ensure we don't check vectors from same premise
            pass
        elif thought_strings[i].split("_")[0] == "[SEP]" or thought_strings[i].split("_")[0] == "[CLS]" or thought_strings[i].split("_")[0] in string.punctuation or thought_strings[i].split("_")[0].split("'")[0] in stop_words:
            pass
            ...
            # print(f"Compare CLS: {thought_strings[i], thought_strings[j], 1 - cosine(thought_vectors[i], thought_vectors[j])}")
            cls_cosine.append(1 - cosine(thought_vectors[i], thought_vectors[j]))
        else:
            # print(f"Compare Token: {thought_strings[i], thought_strings[j], 1 - cosine(thought_vectors[i], thought_vectors[j])}")
            token_cosine.append(1 - cosine(thought_vectors[i], thought_vectors[j]))
            """
            if 1 - cosine(thought_vectors[i], thought_vectors[j]) > 0.90:
                # if thought_strings[j] not in visited:
                # print(thought_strings[i], thought_strings[j], cosine(thought_vectors[i], thought_vectors[j]))
                print(f"Comparer: {thought_strings[i].split('_')[0]} | Comparee: {thought_strings[j].split('_')[0]} | Cosine: {1 - cosine(thought_vectors[i], thought_vectors[j])}")
                # visited.append(thought_strings[i])
                # center_of_venn_diagram.concat([thought_strings[i], thought_strings[j], float(1 - cosine(thought_vectors[i], thought_vectors[j]))])
                # center_of_venn_diagram.append([np.array([thought_strings[i], thought_strings[j]]), float(1 - cosine(thought_vectors[i], thought_vectors[j]))])
                center_of_venn_diagram = center_of_venn_diagram.append({"Word 1": thought_strings[i], "Word 2": thought_strings[j], "Cosine": float(1 - cosine(thought_vectors[i], thought_vectors[j]))}, ignore_index=True)
            """
    if len(token_cosine) > 0 and len(cls_cosine) > 0 and not flag:
        bookkeeping[f"{thought_strings[i]}_token"] += sum(token_cosine)/len(token_cosine)
        bookkeeping[f"{thought_strings[i]}_cls"] += sum(cls_cosine) / len(cls_cosine)
...
# final.to_csv("SemEval/Cosine/thought_token_cls_cosine_test_1.csv", index=False)
print(final.head(10))
