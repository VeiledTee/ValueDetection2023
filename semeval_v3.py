...
# https://mccormickml.com/2019/05/14/BERT-word-embeddings-tutorial/#31-running-bert-on-our-textS
...
for label in list(labels.columns[1:]):
    if os.path.exists(f"SemEval/Cosine/complete_{label.split()[-1].lower()}_cosine.csv"):
        print(f"{label} - Done")
        pass
    else:
        print(label)
        curr = arguments[labels[label] == 1]  # filter by thought
        thought_strings = []
        thought_vectors = []
        ...
        final.to_csv(f"SemEval/Cosine/complete_{label.split()[-1].lower()}_cosine.csv", index=False)
        # print(final.head(10))
        # final = pd.DataFrame(bookkeeping.items(), columns=['Token', 'Cosine'])
